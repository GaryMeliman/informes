	...
	<!-- *********** INICIO ************ -->
		<div class="col-md-8 col-md-offset-2"><?php //<-- MODIFICABLE, para ver distribucion entrar en http://saturn.pinsupreme.com/grid.html?>
			<div class="widget widget-blue" style="height:500px">
				<?php
					$xps_titulo		="Items";//<-- MODIFICABLE					
		            $xps_volver   	="grid_fichas.php?cl=".$_GET['cl']."&pro=".$_GET['pro'];//<-- MODIFICABLE ruta de la página, vacio = sin botón $_SERVER['HTTP_REFERER']
...
<?
	...
	$sql = "SELECT 
			  `f`.`id_ficha`,
			  `f`.`nombre_ficha`,
			  (SELECT `fp`.`id_ficha_proy` 
			  	FROM `fichas_proyectos` `fp` 
			  	WHERE `fp`.`id_ficha` = `f`.`id_ficha` AND `fp`.`id_proyecto` = $proy) AS `id_ficha_proy`
			FROM
			  `fichas` `f`
			ORDER BY
			  `id_ficha_proy` DESC,
...
//danger+times
			...
			$id_ficha_proy = is_null($rs['id_ficha_proy']) ? 0 : $rs['id_ficha_proy'];
			$accion = $id_ficha_proy ? 0 : 1;
...
?>
              ...
              <tr id="<?=$rs['id_ficha']?>" <?=$success?>>
                ...
                <td class="text-center">
                	<span style="cursor: pointer;" class="label label-<?=$crear_s?>" onclick="seleccionar('<?=xps_encriptar($accion)?>','<?=xps_encriptar($id_ficha_proy)?>','<?=xps_encriptar($rs['id_ficha'])?>')">
                		<i class="fa fa-<?=$crear_c?>"></i>
                	</span>
                </td>                
                 <td><?=$rs['nombre_ficha']?></td>
               <td data-toggle="tooltip" data-placement="right" title="" data-original-title="A:<?=$accion?>,I:<?=$id_ficha_proy?>,P:<?=$proy?>,C:<?=$rs['id_ficha']?>"></td>                
              </tr>
<?php
		}
	}
	 ...
	 $(function() {
       ...
       //Guardar
		var dataString = [];
			...
			dataString = dataString.concat({'HdIdFichaProy':HdIdChPro});
	       	dataString = dataString.concat({'HdIdFicha':HiIdChecklist});
