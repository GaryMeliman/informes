		...
		<div class="col-md-12"><?php //<-- MODIFICABLE, para ver distribucion entrar en http://saturn.pinsupreme.com/grid.html?>
			<div class="widget widget-blue" style="height:500px">
				<?php
		            ...
		            $xps_volver   	="grid_fichas.php?cl=".$_GET['cl']."&pro=".$_GET['pro'];
...
<?
	$db	= new MySQL();
	$id_fp = xps_desencriptar($_GET['fp']);//id_chk_proy id_ficha_proy
	$id_f  = xps_desencriptar($_GET['f']);//id_checklist id_ficha
	...
	$sql = "SELECT 
			  ...
			  `fi`.`nombre_item`,
			  `fi`.`porcentaje_item`,
			  (SELECT `fip`.`fecha_ficha_proy` 
			  	FROM `fichas_items_proyecto` `fip` 
			  	WHERE 	`fip`.`id_ficha_item` = `fi`.`id_ficha_item` AND 
			  			`fip`.`id_ficha_proy` = $id_fp) AS `fecha_realizacion`,
			  (SELECT `fip`.`detalle_item_proy` 
			  	FROM `fichas_items_proyecto` `fip` 
			  	WHERE 	`fip`.`id_ficha_item` = `fi`.`id_ficha_item` AND 
			  			`fip`.`id_ficha_proy` = $id_fp) AS `detalle`,
			  (SELECT `fip`.`cierre_item_proy` 
			  	FROM `fichas_items_proyecto` `fip` 
			  	WHERE 	`fip`.`id_ficha_item` = `fi`.`id_ficha_item` AND 
			  			`fip`.`id_ficha_proy` = $id_fp) AS `cierre`
			FROM
			  `fichas_items` `fi`
			WHERE
			  `fi`.`id_ficha` = $id_f
...
<?php
	if(!$accion){
?>
		<button data-toggle="tooltip" data-original-title="Detallar" type="button" class="btn btn-action btn-xs btn-danger" id="create-modal3" onclick="my_modal('Detallar','2',3,'<?=$rs['detalle']?>','<?=$rs['nombre_item']?>','<?=$rs['id_ficha_item']?>','<?=$_GET['fp']?>')">
...
//		if(!$rs['cierre']){
		if(1==0){
?>					
		<button data-toggle="tooltip" data-original-title="Cierre" type="button" class="btn btn-action btn-xs btn-warning" id="create-modal3" onclick="my_modal('Cerrar','0','<?=$rs['id_ficha_item']?>','<?=$_GET['fp']?>')">
							...
							<div class="row">
								<div class="col-md-12">
									<div class="form-group">
										...
										<textarea name="TXTDetalleIP" id="TXTDetalleIP" class="form-control" rows="1"></textarea>
									</div>									
								</div>
							</div>
						</div>
	 ...
	 $(function() {
       ...
       //Guardar
		var dataString = [];
			...
			dataString = dataString.concat({'HdIdFichaProy':'<?=$_GET['fp']?>'});
...
//MODAL 1___________________________________________
    function modal(accion,x,envio,detalle,item,HdIdItemFicha,HdIdFichaProy){
      $("#dialog-form"+x).dialog({modal: true}).dialog('open');
		...
		$("#UploadMedia"+x +" #insert"+x).append("<input type='hidden' name='HdIdFichaProy' id='HdIdFichaProy' value='"+HdIdFichaProy+"'>");
		$("#UploadMedia"+x +" #nomArchivo"+x).append(item);
		$("#TXTDetalleIP").val(detalle);
