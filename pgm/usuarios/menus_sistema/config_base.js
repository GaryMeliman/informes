...
function opciones(id, cont, opcion){

	var configura 	= "config_base.asp?";	
	
//	alert(configura+"id="+ id +"&op="+opcion);

//		return 0;

		location.href=configura+"id="+ id +"&cont="+cont+"&op="+opcion;

}

function tree(level,txt,link,img,id,cont,accion) {
...
//---------------------------------------------------------------------------------------------------------
function treeAdd(level,txt,link,img,id,cont,accion) {
	x = treeData.length;//tamaño elementos del menu
	treeData[x] = new tree(level,txt,link,img,id,cont,accion);
}
...
function makeTree() {
  ...
  for (i=0;i<treeData.length;i++)	{
	...
	if(treeData[i].accion == 1){
		var img_new 	= "";
		var img_del 	= "____<img src='" + imagePath + "/eliminar.png' border=0 width=15 height=15 style='cursor:hand' onclick=opciones('" + treeData[i].id + "','" + treeData[i].cont + "',3) alt='Quita Items' />";	
	}else{
		var img_new 	= "____<img src='" + imagePath + "/nuevo.png' border=0 width=15 height=15 style='cursor:hand' onclick=opciones('" + treeData[i].id + "','" + treeData[i].cont + "',1)  alt='Agrega Items' />";
		var img_del 	= "";
	}
