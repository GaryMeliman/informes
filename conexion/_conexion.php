...
if(isset($_SESSION)){//session ca�da
	cuadro_mensaje('ERROR','Sesi�n Expirada','La sesi�n a caducado, vuelva a logearse',1,0,NULL);
	exit();
}
...
//* Constructor 
	public function __construct($connect_now = true){
		...
		$this->IpServer_Web = "10.0.2.36";//01-11-2014
		...
		if($this->ip == $this->IpServer_Web){
			...
			$this->db_name 	= "db_mnk";// Database			
		}
		else{
			$this->host 	= "localhost"; 		 // Host address
			$this->user 	= "root";	 // User modulone_
			$this->pass 	= "93539514";		 // Password
			$this->db_name 	= "db_mnk";// Database
		}
...
// * CONSULTA A LA BD Y DEVUELVE EL VALOR DE LA CONSULTA
	public function consulta($consulta){
		...
		if($consulta==""){
			$this->error2($this->conexion,"-- SIN CONSULTA --");
			die("CONSULTA VACIA");
			exit();			
		}
...
//* Obtiene una fila como una matriz asociativa, una matriz num�rica o ambos
...
//* funci�n que devuelve el total de consultas realizadas
	public function getTotalConsultas(){
		return $this->total_consultas; 
	}
	
//* Cierre la conexi�n a la base de datos
	public function cerrar_bd(){
		@mysqli_close($this->conexion);  
	}

// * Funci�n para quitar caracteres url
...
// * Funci�n para guardar automaticamente datos desde un formulario a una tabla	
...
//	echo $sql."<hr> Configuracion: ".$configuracion."<br>";
		...
		if($this->num_rows($consulta)>0){
			...
			while($select = $this->fetch_array($consulta)){
				...
				foreach($new_array as $nombre_campoA => $valorA){//compara los valores que son iguales
					//echo $nombre_campoA . "=" . $valorA . "=>". $select[1] ."||". $select['COLUMN_COMMENT'] ."<br>";
					if(strtolower($nombre_campoA) === strtolower($select['COLUMN_COMMENT'])){
...
//							echo $this->reemplaza_caracteres($valorA)."<br>";
			...
			foreach($new_array as $comment_f => $v){
				...
				foreach($orden_key as $comment => $campo){
					//echo $comment."->".$campo."||<br>";
					if(strtolower($comment_f) === strtolower($comment)){
						//echo $campo."><br>";
...
//__________________________________________________________________________________________________
	...
	//__ CONTROL DE ERROR, TABLA SIN ACCION _______________-	
		if($error == 1){
			echo "<b>Error :</b> Se encontro una tabla con acci&oacute;n 0 � 1<br>";
			echo $txt_error;
			return false;
			exit();	
		}
...
// * A�adir caracteres de escape para la importaci�n de datos
...
//* Actualizaci�n de un registro de la base de datos
...
//* Actualizaci�n de multiples registros de la base de datos
...
// * Contar el n�mero de campos de un Resultado
...
// * Obtener la descripci�n del error de la �ltima consulta
	...
	public function error2($conexion,$consulta=""){
		...
		$errores = array('Error','Status',$consulta);
		$tabla  = "<table>";
...
// * Reemplazar caracteres de la consulta en HTML
	public function reemplaza_caracteres3($cadena){
...
// * Reemplazar caracteres de forma inversa sin tildes
	public function reemplaza_caracteres_inversa($cadena){
		...
		$r = array('á','é','í','ó','ú','Á','É','Í','Ó','Ú','ñ','Ñ','','ü','Ü','°');
...
// * Reemplazar caracteres sin tildes
	...
	public function reemplaza_caracteres($string){
